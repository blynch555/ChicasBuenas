...
class flowAPI {
	...
	function __construct() {
		...
		$this->order["Concepto"] = "";
		$this->order["Monto"] = "";
		$this->order["Comision"] = Config::get('kpf.tasa_default');
	...
	*/
	public function build_response($result){
		...
		$data = array();
		$data["status"] = $r;
		$data["c"] = Config::get('kpf.comercio');
	...
	*/
	public function flow_log($message, $type) {
		$HTTPFORWARDED = isset($_SERVER['HTTP_X_FORWARDED_FOR']) ? $_SERVER['HTTP_X_FORWARDED_FOR'] : '';
		$REMOTEADDR = isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '';

		$file = fopen(Config::get('kpf.logPath') . "/flowLog_" . date("Y-m-d") .".txt" , "a+");
		fwrite($file, "[".date("Y-m-d H:i:s.u")." ".$REMOTEADDR." ".$HTTPFORWARDED." - $type ] ".$message . PHP_EOL);
		fclose($file);		   
	}
	...
	private function flow_get_public_key_id() {
		try {
			$fp = fopen(Config::get('kpf.keys') . "/flow.pubkey", "r");
	...
	private function flow_get_private_key_id() {
		try {
			$fp = fopen(Config::get('kpf.keys') . "/comercio.pem", "r");
	...
	private function flow_pack() {
		$comercio = urlencode(Config::get('kpf.comercio'));
		...
		$url_exito = urlencode(Config::get('kpf.url_exito'));
		$url_fracaso = urlencode(Config::get('kpf.url_fracaso'));
		$url_confirmacion = urlencode(Config::get('kpf.url_confirmacion'));
